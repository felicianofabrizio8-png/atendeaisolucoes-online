      ...
      POST: async ({ request }) => {
        ...
        if (body.selected_page_id) {
          try {
            ...
            if (validPageToken) {
              pagePayload.page_access_token = validPageToken;
            } else if (!pageExisting) {
              // Insert sem token: o not-null da coluna requer string; usamos
              // string vazia em vez de propagar token inválido.
