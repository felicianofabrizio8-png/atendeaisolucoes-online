...
export function AICommand({ data }: { data: DashboardData }) {
  ...
  return (
    <section
      ...
      className="col-span-12 overflow-hidden rounded-lg border border-primary/25 bg-card/80 lg:col-span-8"
    >
      <div className="grid gap-5 p-5 md:grid-cols-[minmax(0,1fr)_260px]">
        <div className="min-w-0">
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <h2 id="ai-title" className="truncate font-semibold">
                  Vendedora IA
                </h2>
            ...
            <span className="inline-flex shrink-0 items-center gap-2 rounded-md border border-status-won/25 bg-status-won/10 px-2.5 py-1 text-xs font-medium text-status-won">
        ...
        <div className="border-t pt-4 md:border-l md:border-t-0 md:pl-5 md:pt-0">
          ...
          {recent.length === 0 ? (
            <Empty text="Nenhuma conversa da IA registrada." />
          ) : (
              ...
              {recent.map((item) => (
                <li key={item.id}>
                  <Link
                    ...
                    className="grid grid-cols-[minmax(0,1fr)_auto] gap-2 rounded-md px-2 py-2 hover:bg-accent/50"
                  >
...
function AiMetric({
  ...
  return (
    <div className="min-w-0 rounded-md border bg-background/45 p-3">
      <Icon className={alert ? "h-4 w-4 text-status-warm" : "h-4 w-4 text-primary"} />
