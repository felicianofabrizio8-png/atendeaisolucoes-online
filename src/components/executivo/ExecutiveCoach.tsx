        ...
        {CARDS.map((c) => {
          ...
          return (
            <div key={c.key} className="rounded-xl border border-border p-4 bg-background/50">
              <div className={`inline-flex items-center justify-center h-8 w-8 rounded-lg ${c.tone}`}>
                <Icon className="h-4 w-4" />
              </div>
              <div className="mt-3 text-2xl font-semibold text-foreground">{items.length}</div>
              <div className="text-[11px] text-muted-foreground">{c.label}</div>
              {items[0] && (
                <div className="mt-2 text-xs text-muted-foreground line-clamp-2" title={items[0].title}>
                  {items[0].title}
                </div>
              )}
            </div>
